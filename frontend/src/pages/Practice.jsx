    ...
    <div>
      <section className="page-hero">
        <div className="container">
          <h1>Practice Hub</h1>
          <p>Tutorials, quizzes, and a live code sandbox — build confidence before (or alongside) your full course.</p>
        </div>
      </section>
      ...
      <section className="section section-alt" id="tutorials">
        <div className="container">
          <div className="section-head">
            <h2>Tutorials & Warm-ups</h2>
            <p>Start solving — fundamentals first, then harder patterns.</p>
          </div>
      ...
      <section className="section">
        <div className="container">
          <div className="section-head">
            <h2>Skill Quizzes</h2>
            <p>Quick checks to see where you stand.</p>
          </div>
          <div className="card-grid">
            {quizzes.map((q) => (
              ...
              {(activeQuiz.questions || []).map((q, qi) => (
                <div key={q.question} style={{ marginBottom: 18 }}>
                  ...
                  {(q.options || []).map((opt, oi) => {
                    ...
                    return (
                      <button key={opt} type="button" className={cls} onClick={() => !score && setAnswers({ ...answers, [qi]: oi })}>
                        {opt}
                      </button>
                    );
                  })}
      ...
      <section className="section section-alt" id="compiler">
        <div className="container">
          <div className="section-head">
            <h2>Code Sandbox</h2>
            <p>Practice JavaScript instantly in the browser.</p>
          </div>
          <CodeSandbox />
        </div>
      </section>
      ...
      <section className="section">
        <div className="container">
          <div className="section-head">
            <h2>Free Starter Courses</h2>
            <p>Self-paced intros before you enroll in a paid track.</p>
          </div>
          <div className="card-grid">
            {freeCourses.map((c) => <ProgramCard key={c._id} course={c} />)}
          </div>
          {freeCourses.length === 0 && (
            <p style={{ textAlign: 'center' }}><Link to="/programs">Browse all programs</Link></p>
          )}
        </div>
      </section>
    </div>
